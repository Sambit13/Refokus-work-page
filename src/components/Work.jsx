...
const Work = () => {
  ...
  scrollYProgress.on("change",(data)=>{
   ...
   switch(Math.floor(data*100)){
    ...
    case 6: imageShow([0,1,2,3,4,5]);
                   break;
    case 7: imageShow([0,1,2,3,4,5,6]);
                   break;
  ...
  return (
    <div className='w-full mt-20' >
      <div className='relative max-w-[1260px] mx-auto text-center'>
        <h1 className='text-[30vw] leading-none font-semibold tracking-tight select-none'>Work</h1>
        <div className='absolute w-full h-full top-0'>
          {images.map((elem, index) => (
            elem.isActive && (
              <img key={index} className='absolute w-60 rounded-lg -translate-x-[50%] -translate-y-[50%]' src={elem.url} style={{ top: elem.top, left: elem.left }} />
            )
          ))}
        </div>
      </div>
